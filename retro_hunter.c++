...
/*static*/ class Counts {
    ...
    void get_interesting_junctions(string name, int & read_support, int & pair_support,
        ...
        if(gene_info.size()!=10){
            cerr << "Issue with exon sequence IDs."
                 << "Format should be gid:Gene:Chrom:Start:END/START(+/-)" << endl;
            return;
            //exit(1); // BAD PROGRAMMING PRACTICE TODO: fix
        }

        //otherwise fill in the gene info
        vector<string> gid {gene_info.at(0),gene_info.at(5)};
        vector<string> gene{gene_info.at(1),gene_info.at(6)};
        vector<string> chrom{gene_info.at(2),gene_info.at(7)};
        vector<int>pos{atoi(gene_info.at(3).c_str()),
            atoi(gene_info.at(8).c_str())};
        vector<char> strand{gene_info.at(4).at(gene_info.at(4).size()-2),
            gene_info.at(9).at(gene_info.at(9).size()-2)}; //get the second last character

        bool enough_support = (read_support + pair_support ) >= MIN_COUNTS;
        stringstream junc_pos_formatted;
        junc_pos_formatted << gid[0] << "\t" << chrom[0] << "\t" << pos[0] << "\t"
                             << gid[1] << "\t" << chrom[1] << "\t" << pos[1] ;
        ...
        if( gid[0]==gid[1] && enough_support && not_in_black_list){
            RetroJunction rj;
            rj.gid=gid[0];
            rj.gene=gene[0];
            rj.chrom=chrom[0];
            ...
            retro_results[gid[0]].push_back(rj);
        }
    };
...
public:
    ...
    void print_table(unordered_set<string> & black_list, string outfile ){
        ...
        for(ri=retro_results.begin() ; ri !=retro_results.end() ; ri++){
            ofs << ref_reader->get_gene_name(ri->first) << "\t" ;
...
int mainWIP (int argc, char *argv[])
{
    gzFile fp;
    kseq_t *seq;
    int l;
...
int main (int argc, char *argv[]){
    bam_reader = new BamReader();
    ref_reader = new ReferenceReader();
    ...
    keq = kseq_init(fp); // STEP 3: initialize seq
    while ((l = kseq_read(keq)) >= 0) { // STEP 4: read sequence
        string id(keq->name.s);
        string line (keq->seq.s);
        seqs[id] = line;
    }
    kseq_destroy(keq); // STEP 5: destroy seq
    gzclose(fp); // STEP 6: close the file handler
    ...
    **/
    ...
    if (!bam_reader->setFile(in_filename)) {
        return EXIT_FAILURE;
    }
    ...
    for(int g=0; g<genes.size(); g++){
        ...
        cout << ref_reader->get_gene_name(genes.at(g)) << " " << chrom << ":" << start << "-" << end << endl;
