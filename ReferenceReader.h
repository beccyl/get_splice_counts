...
class ExonPositions {
 ...
 public:
    ...
    ExonPositions() { } // empty constructor, static class causing issues
    ~ExonPositions() {
        starts.empty();
        ends.empty();
    }
    ...
    void get_position(string & chrom, int & start, int &end){
        if(starts.size()==0 || ends.size()==0){
            cerr << "Gene with no exons?" << endl;
            return;
            //exit(1); // bad programming
        }
        chrom=_chrom;
...
class ReferenceReader { //read the fasta
    ...
    void read_fasta( unordered_map<string,string> & _seqs, const string type){
        ...
        for(; seq_itr!=_seqs.end(); seq_itr++){
            ...
            if(is_type){// & right_size){
                ...
                stringstream ss(id);
                string gid,gene,chrom,pos_str,extra_info;
                getline(ss,gid, EXON_ID_DELIM); //TODO: Add
                ...
                id_to_name[gid] = gene; // TODO: this will override several times

                if(ref_pos[gid].add_boundary(chrom,atoi(pos_str.c_str()),type,extra_info.at(extra_info.size()-2))) {
    ...
    // copy constructor
    ReferenceReader(const ReferenceReader& other) { }

    // copy assignment
    ReferenceReader& operator=(const ReferenceReader& other)
    {
        if (this != &other)
        {
        }
    ...
    // move constructor
    ReferenceReader(ReferenceReader && other) noexcept { }

    // move assignment
    ReferenceReader& operator=(ReferenceReader&& other) noexcept {
        return *this;
    }
 ...
 public:
    ...
    ~ReferenceReader() {
        ref_pos.empty();
        id_to_name.empty();
        junc_seq_start.empty();
        junc_seq_end.empty();
    }


    string get_gene_name(string gid) {
        return id_to_name[gid];
    }
    ...
    void get_gene_coords(string gid, string & chrom, int & start, int &end){
        ref_pos[gid].get_position(chrom, start, end);
    }

    string get_closest_junction(string gid, string chrom, int r1_end, int r2_start){
        int jend,jstart;
        if(ref_pos[gid].get_closest_junction(chrom,r1_end,r2_start,jend,jstart)){
            stringstream junc_ss;
            string strand="(+)";
            if(jstart<jend) strand="(-)";
                junc_ss << gid << EXON_ID_DELIM << get_gene_name(gid) << EXON_ID_DELIM << chrom << EXON_ID_DELIM << jend << END_LABEL << strand ;
                junc_ss << EXON_ID_DELIM ;
                junc_ss << gid << EXON_ID_DELIM << get_gene_name(gid) << EXON_ID_DELIM << chrom << EXON_ID_DELIM << jstart << START_LABEL << strand ;
            return junc_ss.str();
        }
        else
            return "";
